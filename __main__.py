    ...
    def run(self):
        ...
        try:
            while queue:
                ...
                if queue:
                    CONDITION.acquire()
                    catch_tuple = queue.pop()
                    CONDITION.release()
                    news_id = catch_tuple[1].split('/')[-1]
                    print(news_id)
                    # print(catch_tuple)
                    print(self.threadname+": the url will be read:"+catch_tuple[1])
                    if news_id not in visited:
                        visited |= {news_id}
                        print("before read_one_news")
                        print("++++++++++++++++++2+++++++++++++++++++++")
                        self.news_unit = self.read_one_news(catch_tuple)
                        print("before insert")
...
if __name__ == "__main__":
    ...
    # url = ("slide_4_704_157938.html","http://slide.ent.sina.com.cn/star/slide_4_704_157938.html")
